function  Create_Porosity_Layers(Porosity_template, Porosity_values, WEC_dimension, Wavedir, T)
...
P_lat=WEC_lat:WEC_lat+P_width; %Gets porosity x and y locational range
...
for n=1:length(Porosity_values)   
    ...
    %save_dfs2
    Output_file=[Wavedir(1:end-16) 'Porosity\Porosity_test' sprintf('%02.f',T) '\Porosity_' sprintf('%.3f',Porosity_values(n)) '.dfs2' ] ;    
